    ...
    public partial class Example : Form
    {
        ...
        private async Task<ExecutionMessageEventData> GetExecutionEventDataAsync(UIMessageInstance uiMessage)
        {
            try
            {
                ...
                var prevStepIndex = (await _clients.SequenceContextClient
                ...
                if (prevStepIndex >= 0)
                {
                    ...
                    executionData.PrevStep = await GetStepDataAsync(prevStep);
                    executionData.PrevStep.StepIndex = prevStepIndex;
                }

                int currStepIndex = (await _clients.SequenceContextClient
                ...
                if (currStepIndex > 0)
                {
                    ...
                    executionData.CurrStep = await GetStepDataAsync(currentStep);
                    executionData.CurrStep.StepIndex = currStepIndex;
                }

                int nextStepIndex = (await _clients.SequenceContextClient
                ...
                if (nextStepIndex >= 0)
                {
                    ...
                    executionData.NextStep = await GetStepDataAsync(nextStep);
                    executionData.NextStep.StepIndex = nextStepIndex;
                }
        ...
        private async Task<StepData> GetStepDataAsync(StepInstance step)
        {
            ...
            {
                StepName = (await _clients.StepClient
                    .Get_NameAsync(new Step_Get_NameRequest
                    {
                        Instance = step
                    })).ReturnValue,

                StepResultStatus = (await _clients.StepClient
                    .Get_ResultStatusAsync(new Step_Get_ResultStatusRequest
                    {
                        Instance = step
                    })).ReturnValue,

                StepTypeName = (await _clients.StepTypeClient
                    ...
                    if (activeExecution != null)
                    {
                        switch (uiMessageCode)
                        {
                            ...
                            case UIMessageCodes.UimsgStartExecution:
                                {
                                    ...
                                    eventData.MessageCode = (int)uiMessageCode;
                            ...
                            case UIMessageCodes.UimsgEndExecution:
                                {
                                    ...
                                    //if (traceExecution)
                                    {
                                        ...
                                        {
                                            LogTraceMessage(Invariant($"\tExecution {executionInstance.Id} (internal id {executionId}) is done running.") + Environment.NewLine);
                                        }
                            ...
                            case UIMessageCodes.UimsgTrace:
                                {
                                    ...
                                    if (traceExecution)
                                    {
                                        ...
                                        if (eventData.PrevStep?.StepIndex >= 0)
                                        {
                                            ...
                                            string statusFormatted = string.Format("{0,-" + StatusLength + "}", eventData.PrevStep?.StepResultStatus);
                                            message += Invariant($"{statusFormatted}  Step {eventData.PrevStep?.StepName}");

                                            if (eventData.PrevStep?.StepResultStatus == _errorResultStatusConstant)
                                            {
                            ...
                            case UIMessageCodes.UimsgAbortingExecution:
                                {
                                    LogLine("Aborting Execution");

                                    //var eventData = new ExecutionMessageEventData
                                    //{
                                    //    MessageCode = (int)uiMessageCode,
                                    //    ExecutionId = (await GetExecutionAsync(uiMessageEvent.Msg)).Id
                                    //};

                                    var eventData = await GetExecutionEventDataAsync(uiMessageEvent.Msg);

                                    LogTraceMessage($"\tAborting Execution: active execution:{activeExecution.Id}, event execution: {eventData.ExecutionId}\n");
                                    LogMessageStepData(eventData);
                                }
                                break;
                            case UIMessageCodes.UimsgTerminatingExecution:
                                {
                                    LogLine("Terminating Execution");
                                    ...
                                    LogTraceMessage($"\tTerminating Execution: active execution:{activeExecution.Id}, event execution: {eventData.ExecutionId}\n");
                                    LogMessageStepData(eventData);
                                }
                                break;
                        }
        ...
        {
            LogTraceMessage($"\tPrev step: Index = {eventData.PrevStep?.StepIndex ?? -1}, Name = {eventData.PrevStep?.StepName ?? ""} Type = {eventData.PrevStep?.StepTypeName ?? ""}, Status = {eventData.PrevStep?.StepResultStatus ?? ""}\n");
            LogTraceMessage($"\tCurr step: Index = {eventData.CurrStep?.StepIndex ?? -1}, Name = {eventData.CurrStep?.StepName ?? ""} Type = {eventData.CurrStep?.StepTypeName ?? ""}, Status = {eventData.CurrStep?.StepResultStatus ?? ""}\n");
            LogTraceMessage($"\tNext step: Index = {eventData.NextStep?.StepIndex ?? -1}, Name = {eventData.NextStep?.StepName ?? ""} Type = {eventData.NextStep?.StepTypeName ?? ""}, Status = {eventData.NextStep?.StepResultStatus ?? ""}\n");
        }
        ...
        private void InitializeEnableTracingOption()
        {
            ...
            var execMask = _clients.StationOptionsClient.Get_ExecutionMask(
                new StationOptions_Get_ExecutionMaskRequest
                {
                    Instance = stationOptions,
                }).ReturnValue;

            execMask = (ExecutionMask.ExecMaskDefaultExecutionMask | ExecutionMask.ExecMaskTraceAllThreads)
                   & ~(ExecutionMask.ExecMaskTraceIntoSetupCleanup | ExecutionMask.ExecMaskTraceIntoEntryPoints);

            _clients.StationOptionsClient.Set_ExecutionMask(
                new StationOptions_Set_ExecutionMaskRequest
                {
                    Instance = stationOptions,
                    Val = execMask
                });

            execMask = _clients.StationOptionsClient.Get_ExecutionMask(
                new StationOptions_Get_ExecutionMaskRequest
                {
                    Instance = stationOptions,
                }).ReturnValue;
